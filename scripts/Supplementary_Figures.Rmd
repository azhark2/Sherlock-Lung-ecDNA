...
run_logistic_regression <- function(feature, data, smoking = TRUE) {
  ...
  base_formula <- paste("ecDNA. ~", feature, "+ Gender + Tumor_Purity + Age + Ancestry + wGII")# + Histology")
  ...
  #if feature is chromothripsis, then subset the data for only ecDNA that are complex or multi-chromosomal
  if (feature == "Chromothripsis") {
    #filter for ecDNA.class that are not simple
    data <- data %>% filter(ecDNA.class != "Simple")
  }
...
features = c("WGD_Status", "Telseq_TL_Normal", "TMB", "mt_copy_number_median", "hypoxia_score_median", "Chromothripsis")
...
for (feature in features) {
  ...
  model_summary <- run_logistic_regression(feature, data, smoking = FALSE)
...
#results_summary %>% write.table("~/iCloud/dev/Sherlock-Lung/results2/log-reg-results2/non-smokers-LUAD-genomic.tsv", row.names=FALSE, sep='\t', quote=FALSE)
...
non_smokers_plot <- create_volcano_plot(results_summary, "Never-smoker LUAD (n=737)", "Adjusted by age, sex, ancestry, purity")
...
smokers_plot <- create_volcano_plot(results_summary, "Smoker LUAD (n=286)", "Adjusted by age, sex, ancestry, purity")

library(patchwork)

# Combine plots side by side with patchwork
combined_plot <- non_smokers_plot | smokers_plot

# Add the title
combined_plot <- combined_plot + 
  plot_annotation(
    title = "Genomic features associated with ecDNA",
...
```{r SUPP FIGURE 5d -- EGFRL858 Bar}

create_barplot <- function(data, x_var, y_var = "ecDNA_status", palette_colors = c("ecDNA+"="red", "ecDNA-" = "gray"), title = NULL, include_chisq = FALSE) {
...
p <- ggplot(data_summary, aes(x = !!x_var_sym, y = percentage, fill = !!y_var_sym)) +
   geom_bar(stat = "identity", position = "stack", color = "black") +
   labs(y = "Percentage", subtitle="OR=1.78, q=0.05, Adjusted by age, sex, ancestry, histology, purity", title="EGFR L858R mutation in never-smokers") +
   coord_cartesian(ylim = c(-10, 100)) +
   theme_minimal() +
   scale_fill_manual(values = palette_colors) +
   theme(
...
#read data
m <- read.csv("..data/EGFR_L591R_mutations.tsv", sep="\t", header=TRUE)
data <- read.csv("..data/log-reg-table5.tsv", sep="\t", header=TRUE)
#drop smoking is Unknown
data <- data %>%
  filter(Smoking != "Unknown")

#make a new column called EGFR_L591R and if the tumor barcode is present in m, "Yes", else, "No"

data$EGFR_L591R <- ifelse(data$Tumor_Barcode %in% m$Tumor_Barcode, "Yes", "No")
...
#randomly drop one row from the data that has EGFR_L591R "No"
# nonsmokers <- nonsmokers %>%
#   slice(sample(nrow(nonsmokers), nrow(nonsmokers)-1))

#create barplot 
p <- create_barplot(nonsmokers, "EGFR_L591R", "ecDNA.", title = "EGFR L591R Mutation", include_chisq = FALSE)
