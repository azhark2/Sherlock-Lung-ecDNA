...
#change directory 
PATH_TO_DATA_FOLDER = "~/iCloud/dev/Sherlock-Lung-ecDNA-backup/data/" #change this to point to path of data folder
PATH_TO_METADATA_FOLDER = "~/iCloud/dev/Sherlock-Lung-ecDNA-backup/metadata/" #change this to point to path of metadata folder
```
...
```{r FIGURE 1E -- NON-SMOKERS AND SMOKERS EPI PLOT W/ ALL VARIABLES INCLUDED}
...
data <- read.delim(file.path(PATH_TO_DATA_FOLDER, "log-reg-table5.tsv"),
...
metadata <- read.delim(file.path(PATH_TO_DATA_FOLDER, "sherlock_1217_metadata_updated.tsv"),
                       header = TRUE, check.names = FALSE)
...
s <- read.delim(file.path(PATH_TO_DATA_FOLDER, "ecDNA_amplicon_table_content.tsv"),
                       header = TRUE)

metadata <- read.delim(file.path(PATH_TO_METADATA_FOLDER, "sherlock_1217_metadata.tsv"),
                       header = TRUE)
...
PATH_TO_DATA_FOLDER="/Users/akhandek/iCloud/dev/Sherlock-Lung-ecDNA-backup/data"
#derive amplfication classes for survival analysis using AA ecDNA calls and Battenberg CN calls
s <- read.delim(file.path(PATH_TO_DATA_FOLDER, "log-reg-table5.tsv"),
                header = TRUE, check.names = FALSE)

bb_dir <- file.path(PATH_TO_DATA_FOLDER, "Battenberg_CN")

files   <- list.files(bb_dir, pattern = "\\.bed$", full.names = TRUE)
samples <- tools::file_path_sans_ext(basename(files))

amp_class <- character(length(files))

for (i in seq_along(files)) {
  df <- read.table(files[i], header = TRUE, sep = "\t", check.names = FALSE, comment.char = "")
  
  total_cn <- df[["clone_nMaj"]] + df[["clone_nMin"]]
  length <- df[["endpos"]] - df[["startpos"]]
  
  if (any(total_cn >= 3, na.rm = TRUE) ) {
    amp_class[i] <- "chrAmp"
  } else {
    amp_class[i] <- "No amp"
  }
...
res <- data.frame(Sample = samples, AmpClass = amp_class, stringsAsFactors = FALSE)



#merge with res by Tumor_Barcode and Sample
s <- merge(s, res, by.x="Tumor_Barcode", by.y="Sample", all.x=TRUE)


#if ecDNA_status == 1, then change Classification2 to "ecDNA"
s$Classification2 <- ifelse(s$ecDNA_status == 1, "ecDNA", s$AmpClass)

s$Classification2 <- factor(s$Classification2, levels = c('chrAmp', 'No amp', "ecDNA"))
...
create_survival_plot <- function(data, time_col, status_col, grouping_var, highlight_level, title, xlim, highlight_level2 = NULL) {
  ...
  if (!is.null(highlight_level2) && highlight_level2 %in% strata_names) {
    custom_palette[highlight_level2] <- "gray"  
  }
...
smokers <- read.delim(file.path(PATH_TO_DATA_FOLDER, "smokers_survival_table.tsv"),
                       header = TRUE, check.names = FALSE)

#remove NA from Classification2 column in smokers and "" 
smokers <- smokers %>%
  dplyr::filter(!is.na(Classification2) & Classification2 != "")

#age as numeric
smokers$Age <- as.numeric(smokers$Age)

smokers$Classification2 <- factor(smokers$Classification2, levels = c('chrAmp', 'No amp', "ecDNA"))
smokers$Population <- factor(smokers$Population, levels = c('EAS', 'EUR', 'Other'))
smokers$Histology <- factor(smokers$Histology, levels = c('Adenocarcinoma', "Squamous cell carcinoma", "Carcinoid tumor", "Other"))
smokers$Gender <- factor(smokers$Gender, levels = c('Male', 'Female'))
smokers$Stage_group <- factor(smokers$Stage_group, levels = c('I', 'II', 'III', 'IV'))

#subset for only Gender + Age + Population + Stage_group + Histology + wGII columns in smokers
smokers <- smokers %>%
  dplyr::select(Tumor_Barcode, Gender, Survival_Month_5yr, Death_5yr, Classification2, Age, Population, Stage_group, Histology, wGII)


nonsmoker_plot <- create_survival_plot(nonsmokers, "Survival_Month_5yr", "Death_5yr", "Classification2", "ecDNA", "Survival by Classification", 60, highlight_level2 = "No amp")

smoker_plot <- create_survival_plot(smokers, "Survival_Month_5yr", "Death_5yr", "Classification2", "ecDNA", "Survival by Classification", 60, highlight_level2 = "No amp")
