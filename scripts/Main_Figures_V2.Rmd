...
classify <- function(f) {
  ...
  if (any((as.integer(df$clone_nMaj) + as.integer(df$clone_nMin)) >= 4, na.rm = TRUE)) "chrAmp" else "No amp"
}

res <- data.frame(Sample = samples,
                  Classification2 = vapply(files, classify, character(1)),
