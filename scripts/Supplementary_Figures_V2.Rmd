...
# data <- data %>%
#   mutate(
#     telhunter = ifelse(telhunter_TL >= median(telhunter_TL, na.rm=TRUE), 1, 0),
#     telomerecat = ifelse(telomerecat_TL >= median(telomerecat_TL, na.rm=TRUE), 1, 0)
#   )
...
p <- ggplot(results_summary, aes(x = log2(Odds), y = Features)) +
  geom_point(aes(color = ifelse(Odds > 3, "red", "black")), size = 3) +
  geom_errorbarh(aes(xmin = log2(Lower.CI), xmax = log2(Upper.CI), color = ifelse(Odds > 3, "red", "black")), height = 0.2) +
  geom_text(aes(label = paste0("OR = ", Odds, ", q= ", formatC(Corrected.p.value, format = "e", digits = 2))), vjust = -0.7, nudge_y = 0.2, nudge_x = 0.25, size=3.2) +
  theme_minimal() +
  labs(
    title = "ecDNA status",
...
formula <- paste("ecDNA_status ~", feature, "+ Gender + Tumor_Purity + Age + Ancestry + Histology")# + Histology")
